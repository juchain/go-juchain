...
Usage: go run buildbuild.go <command> <command flags/arguments>
...
var (
	...
	debExecutables = []debExecutable{
		{
			Name:        "cmd",
			Description: "Infinet CLI client.",
		},
	}
...
func main() {
	...
	if _, err := os.Stat(filepath.Join("", "build.go")); os.IsNotExist(err) {
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Where to upload the source package (usually "ppa:infinetio/infinetio")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "juchain-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func newDebMetadata(distro, author string, env build.Environment, t time.Time) debMetadata {
	if author == "" {
		...
		author = "Infinet Builds <admin@infinet.org>"
	}
	return debMetadata{
		Env:         env,
		Author:      author,
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "juchain.exe" {
			gethTool = file
		} else {
	...
	// first section contains the juchain binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Juchain":     gethTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.juchain.nsi", filepath.Join(*workdir, "juchain.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("juchain-" + archiveBasename(*arch, env) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "juchain.nsi"),
	)
