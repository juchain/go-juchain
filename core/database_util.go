...
var (
	...
	configPrefix   = []byte("ethereum-config-") // config prefix for the store

	// Chain index prefixes (use `i` + single byte to avoid mixing data types).
	BloomBitsIndexPrefix = []byte("iB") // BloomBitsIndexPrefix is the data table of a chain indexer to track its progress

	// used by old store, now only used for conversion
	oldReceiptsPrefix = []byte("receipts-")
	oldTxMetaSuffix   = []byte{0x01}

	ErrChainConfigNotFound = errors.New("ChainConfig not found") // general config not found error

	preimageCounter    = metrics.NewRegisteredCounter("store/preimage/total", nil)
	preimageHitCounter = metrics.NewRegisteredCounter("store/preimage/hits", nil)
)
...
func WriteCanonicalHash(db store.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db store.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db store.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db store.Putter, hash common.Hash) error {
...
func WriteTrieSyncProgress(db store.Putter, count uint64) error {
...
func WriteHeader(db store.Putter, header *types.Header) error {
...
func WriteBody(db store.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db store.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db store.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db store.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db store.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db store.Putter, block *types.Block) error {
...
func WriteBloomBits(db store.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db store.Database) store.Database {
	return store.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db store.Database, number uint64, preimages map[common.Hash][]byte) error {
...
// GetBlockChainVersion reads the version number from store.
...
// WriteBlockChainVersion writes vsn as the version number to store.
func WriteBlockChainVersion(db store.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db store.Putter, hash common.Hash, cfg *config.ChainConfig) error {
