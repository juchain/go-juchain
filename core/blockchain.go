...
import (
	...
	"github.com/juchain/go-juchain/common/mclock"
...
// Processor.
func NewBlockChain(db store.Database, cacheConfig *CacheConfig, chainConfig *config.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) loadLastState() error {
	...
	log.Info("Loaded most recent local header", "bchain", bc.Genesis().DAppID().String(), "number", currentHeader.Number, "hash", currentHeader.Hash(), "difficulty", headerTd)
	log.Info("Loaded most recent local full block", "bchain", bc.Genesis().DAppID().String(), "number", currentBlock.Number(), "hash", currentBlock.Hash(), "difficulty", blockTd)
	log.Info("Loaded most recent local fast block", "bchain", bc.Genesis().DAppID().String(), "number", currentFastBlock.Number(), "hash", currentFastBlock.Hash(), "difficulty", fastTd)
...
// until one with associated state is found. This is needed to fix incomplete store
...
func SetReceiptsData(config *config.ChainConfig, block *types.Block, receipts types.Receipts) error {
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		case err == ErrPrunedAncestor:
			// Block competing with the canonical chain, store in the store, but don't process
