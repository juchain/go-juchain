...
type ChainIndexer struct {
	chainDb  store.Database      // Chain database to index the data from
	indexDb  store.Database      // Prefixed table-view of the store to write index metadata into
...
func NewChainIndexer(chainDb, indexDb store.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
