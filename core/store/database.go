...
func NewLDBDatabase(file string, cache int, handles int) (*LDBDatabase, error) {
	...
	// Open the store and recover any potential corruptions
	...
	// (Re)check for errors and abort if opening of the store failed
	if err != nil {
		return nil, err
	}
	return &LDBDatabase{
...
func (db *LDBDatabase) meter(refresh time.Duration) {
	...
	for i := 1; ; i++ {
		...
		if db.writeDelayNMeter != nil {
			...
			// If a warning that store performance is laggy has been displayed,
