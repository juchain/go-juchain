...
func genValueTx(nbytes int) func(int, *BlockGen) {
	return func(i int, gen *BlockGen) {
		...
		tx, _ := types.SignTx(types.NewTransaction(gen.TxNonce(benchRootAddr), toaddr, big.NewInt(1), gas, nil, data), types.DefaultSigner{}, benchRootKey)
		gen.AddTx(tx)
	}
}
...
func genTxRing(naccounts int) func(int, *BlockGen) {
	...
	return func(i int, gen *BlockGen) {
		...
		for {
			...
			tx, _ = types.SignTx(tx, types.DefaultSigner{}, ringKeys[from])
			gen.AddTx(tx)
			from = to
		}
	}
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	if !disk {
		...
		db, err = store.NewLDBDatabase(dir, 128, 128)
		if err != nil {
	...
	chainman, _ := NewBlockChain(db, nil, gspec.Config, nil, vm.Config{})
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
// into a database.
func makeChainForBench(db store.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		...
		db, err := store.NewLDBDatabase(dir, 128, 1024)
		if err != nil {
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, nil, config.TestChainConfig, nil, vm.Config{})
		if err != nil {
