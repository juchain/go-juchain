...
func TestCacheUnload(t *testing.T) {
	...
	root, _ := trie.Commit(nil)
	trie.db0.Commit(root, true)

	// Commit the trie repeatedly and access key1.
	// The branch containing it is loaded from DB exactly two times:
	// in the 0th and 6th iteration.
	db0 := &countingDB{Database: trie.db0.diskdb, gets: make(map[string]int)}
	trie, _ = New(root, NewDatabase(db0))
	...
	for dbkey, count := range db0.gets {
		if count != 2 {
			t.Errorf("store key %x loaded %d times, want %d times", []byte(dbkey), count, 2)
		}
	}
}
