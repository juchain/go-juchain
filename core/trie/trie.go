...
// New creates a trie with an existing root node from store.
//
// If root is the zero hash or the sha3 hash of an empty string, the
// trie is initially empty and does not require a database. Otherwise,
// New will panic if store is nil and returns a MissingNodeError if root does
// not exist in the database. Accessing the trie loads nodes from store on demand.
...
func (t *Trie) Commit(onleaf LeafCallback) (root common.Hash, err error) {
	...
	hash, cached, err := t.hashRoot(t.db0, onleaf)
	if err != nil {
