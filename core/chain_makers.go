...
// parent will be the provided parent. store is used to store
...
func GenerateChain(config0 *config.ChainConfig, parent *types.Block, engine consensus.Engine, db store.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	if config0 == nil {
	...
	genblock := func(i int, parent *types.Block, statedb *state.StateDB) (*types.Block, types.Receipts) {
		...
		if b.engine != nil {
			...
			// Write state changes to store
			root, err := statedb.Commit(config0.IsEIP158(b.header.Number))
			if err != nil {
...
// header only chain.
func newCanonical(engine consensus.Engine, n int, full bool) (store.Database, *BlockChain, error) {
	// Initialize a fresh chain with only a genesis block
	gspec := new(Genesis)
	db, _ := store.NewMemDatabase()
	genesis := gspec.MustCommit(db)

	blockchain, _ := NewBlockChain(db, nil, config.AllEthashProtocolChanges, engine, vm.Config{})
...
func makeHeaderChain(parent *types.Header, n int, engine consensus.Engine, db store.Database, seed int) []*types.Header {
...
func makeBlockChain(parent *types.Block, n int, engine consensus.Engine, db store.Database, seed int) []*types.Block {
