...
// Service implements an JuchainService netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	server *p2p.Server      // Peer-to-peer server to retrieve networking infos
	eth    *JuchainService  // Full JuchainService service if monitoring a full node
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["eth"]; info != nil {
		network = fmt.Sprintf("%d", info.(*NodeInfo).Network)
		protocol = fmt.Sprintf("eth/%d", ProtocolVersions[0])
	} else {
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	if s.eth != nil {
		mining =  false //s.eth.Miner().Mining()
		hashrate = 0 //int(s.eth.Miner().HashRate())
