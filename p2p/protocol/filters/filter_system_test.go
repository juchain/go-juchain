...
import (
	...
	ethereum "github.com/juchain/go-juchain"
...
func (b *testBackend) ChainDb() store.Database {
	return b.db
}
			...
			case request := <-requests:
				...
				for i, section := range task.Sections {
					if rand.Int()%4 != 0 { // Handle occasional missing deliveries
						head := core.GetCanonicalHash(b.db, (section+1)*config.BloomBitsBlocks-1)
...
func TestBlockSubscription(t *testing.T) {
	...
	var (
		...
		chain, _    = core.GenerateChain(config.TestChainConfig, genesis, nil, db, 10, func(i int, gen *core.BlockGen) {})
