...
import (
	...
	"github.com/juchain/go-juchain/p2p/protocol/fetcher"
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing P2P peer", "peer", id)

	// Unregister the peer from the downloader and P2P peer set
...
func (pm *ProtocolManager) Start(maxPeers int) {
	log.Info("Starting P2P protocol")
	...
	pm.dposManager.Start(maxPeers)
}

func (pm *ProtocolManager) Stop() {
	log.Info("Stopping P2P protocol")
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Info("P2P peer connected", "name", p.Name())

	// Execute the P2P handshake
	var (
	...
	if err := p.Handshake(pm.networkId, td, hash, genesis.Hash()); err != nil {
		p.Log().Info("P2P handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("P2P peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Info("P2P message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the P2P sub-protocol metadata
