...
const (
	// bloomServiceThreads is the number of goroutines used globally by an JuchainService
...
func (eth *JuchainService) startBloomHandlers() {
			...
			for {
				...
				case request := <-eth.bloomRequests:
					...
					for i, section := range task.Sections {
						head := core.GetCanonicalHash(eth.chainDb, (section+1)*config.BloomBitsBlocks-1)
...
// for the JuchainService header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size uint64 // section size to generate bloombits for

	db  store.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db store.Database, size uint64) *core.ChainIndexer {
	...
	table := store.NewTable(db, string(core.BloomBitsIndexPrefix))
