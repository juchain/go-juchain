...
// PublicJuchainAPI provides an API to access Juchain related information.
...
type rawWallet struct {
	...
	Accounts []account.Account `json:"account,omitempty"`
}
...
func (s *PrivateAccountAPI) SendTransaction(ctx context.Context, args SendTxArgs, passwd string) (common.Hash, error) {
	if args.Nonce == nil {
		...
		// the same nonce to multiple account.
...
//   keccak256("\x19Juchain Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Juchain Signed Message:\n%d%s", len(data), data)
...
// PublicBlockChainAPI provides an API to access the Juchain blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
...
func (s *PublicBlockChainAPI) EstimateGas(ctx context.Context, args CallArgs) (hexutil.Uint64, error) {
	...
	if uint64(args.Gas) >= config.TxGas {
		hi = uint64(args.Gas)
	} else {
...
func (s *PublicTransactionPoolAPI) sign(addr common.Address, tx *types.Transaction) (*types.Transaction, error) {
	...
	return wallet.SignTx(account0, tx, chainID)
}
...
func (args *SendTxArgs) toTransaction() *types.Transaction {
	...
	return types.NewDAppTransaction(args.DAppID, uint64(*args.Nonce), uint64(*args.Gas), (*big.Int)(args.GasPrice), input)
}
...
func (s *PublicTransactionPoolAPI) SendTransaction(ctx context.Context, args SendTxArgs) (common.Hash, error) {
	...
	signed, err := wallet.SignTx(account0, tx, chainID)
	if err != nil {
		return common.Hash{}, err
	}
	return submitTransaction(ctx, s.b, signed)
...
func (s *PublicTransactionPoolAPI) Sign(addr common.Address, data hexutil.Bytes) (hexutil.Bytes, error) {
	...
	signature, err := wallet.SignHash(account0, signHash(data))
	if err == nil {
		signature[64] += 27 // Transform V from 0/1 to 27/28 according to the yellow paper
	}
...
// PublicDebugAPI is the collection of Juchain APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Juchain APIs exposed over the private
// debugging endpoint.
	...
	return []rpc.API{
		{
			Namespace: "block",
			Version:   "1.0",
			Service:   NewPublicJuchainAPI(apiBackend),
			Public:    true,
		}, {
			Namespace: "block",
			Version:   "1.0",
