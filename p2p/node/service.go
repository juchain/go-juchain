...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (store.Database, error) {
	if ctx.Config.DataDir == "" {
		return store.NewMemDatabase()
	}
	db, err := store.NewLDBDatabase(ctx.Config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
