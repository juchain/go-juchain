...
type tester struct {
	workspace string
	stack     *node.Node
	ethereum  *protocol.JuchainService
	console   *Console
...
func newTester(t *testing.T, confOverride func(*protocol.Config)) *tester {
	...
	// Create a networkless protocol stack and start an JuchainService service within
	...
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return protocol.New(ctx, ethConf) }); err != nil {
		t.Fatalf("failed to register JuchainService protocol: %v", err)
	}
	...
	var ethereum *protocol.JuchainService
